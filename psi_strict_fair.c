...
// strictly enforce capacity by removing random items
void enforce_capacity(const KP *kp, int *sel) {
...
// perturb without breaking capacity (after perturbation, enforce)
void perturb(const KP *kp, int *sel, double N) {
    ...
    enforce_capacity(kp, sel);
}

int main() {
    srand(12345);   // master seed for whole benchmark (not used for problem generation)
    FILE *csv = fopen("psi_strict_fair_learning.csv", "w");
    ...
    for (int seed = 0; seed < INSTANCES; seed++) {
        ...
        // PSI-CORE (strict capacity enforcement)
        greedy(&kp, sel);
        while (ls(&kp, sel));
        eval(&kp, sel, &val, &wgt);
        ...
        for (int iter = 0; iter < 100; iter++) {
            ...
            perturb(&kp, sel, N_dim);   // perturb already enforces capacity
            while (ls(&kp, sel));       // local search never violates capacity
            eval(&kp, sel, &val, &wgt);
    ...
    printf("STRICT FAIR BENCHMARK (capacity=%d, instances=%d)\n", CAP, INSTANCES);
    printf("Method   | Avg Value\n");
    printf("GR-B     | %9.1f\n", sum_grb/INSTANCES);
    printf("GR-LS    | %9.1f\n", sum_grls/INSTANCES);
    printf("PSI-CORE | %9.1f\n", sum_psi/INSTANCES);
    printf("PSI vs GR-LS: better=%d  equal=%d  worse=%d\n", better, equal, worse);
    printf("Clean CSV: psi_strict_fair_learning.csv\n");
    return 0;
}
