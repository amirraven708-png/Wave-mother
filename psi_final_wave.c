...
// Intelligent repair: remove lowest value/weight ratio items until feasible
void smart_repair(const KP *kp, int *sel) {
    int val, wgt; eval(kp, sel, &val, &wgt);
    while (wgt > kp->cap) {
        int worst = -1; double min_ratio = 1e9;
        for (int i = 0; i < kp->n; i++) {
            if (sel[i]) {
                double r = (double)kp->items[i].v / kp->items[i].w;
                if (r < min_ratio) { min_ratio = r; worst = i; }
            }
        }
        if (worst >= 0) { sel[worst] = 0; eval(kp, sel, &val, &wgt); }
        else break;
    }
}

// Wave‑based perturbation with smart repair
void wave_perturb(const KP *kp, int *sel, double N) {
    int flips = (int)(N * 0.7);  // slightly more aggressive
    if (flips < 1) flips = 1;
...
int main() {
    srand(12345);
    FILE *csv = fopen("psi_final_wave.csv", "w");
    ...
    for (int seed = 0; seed < INSTANCES; seed++) {
        ...
        // PSI-CORE with wave exploration
        greedy(&kp, sel);
        while (ls(&kp, sel));
        eval(&kp, sel, &val, &wgt);
        ...
        for (int iter = 0; iter < 100; iter++) {
            // PSI breathing (identical to previous versions)
            double rp = residual;
            ...
            wave_perturb(&kp, sel, N_dim);   // intelligent perturbation
            while (ls(&kp, sel));            // polish
            eval(&kp, sel, &val, &wgt);

            if (val > best_val && wgt <= kp.cap) {
                best_val = val;
                memcpy(best_sel, sel, sizeof(sel));
                residual *= 0.85;
        ...
        eval(&kp, best_sel, &val, &wgt);
        sum_psi += val;
        fprintf(csv, "%d,PSI,%d,%d\n", seed, val, wgt);

        if (val > grls_val) better++;
        else if (val == grls_val) equal++;
        else worse++;
    }

    fclose(csv);

    printf("FINAL WAVE BENCHMARK (capacity=%d, instances=%d)\n", CAP, INSTANCES);
    printf("Method   | Avg Value\n");
    printf("GR-B     | %9.1f\n", sum_grb/INSTANCES);
    printf("GR-LS    | %9.1f\n", sum_grls/INSTANCES);
    printf("PSI-CORE | %9.1f\n", sum_psi/INSTANCES);
    printf("PSI vs GR-LS: better=%d  equal=%d  worse=%d\n", better, equal, worse);
    printf("Clean CSV: psi_final_wave.csv\n");
    return 0;
}
