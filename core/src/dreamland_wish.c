...
static double int_to_phase(uint64_t phase_int) {
    return (double)(phase_int % 10000) / 10000.0 * 2.0 * M_PI;
}
...
static int wish_pattern(dreamland_t *dl, const wish_t *wish, dream_result_t *results, int max_results) {
    ...
    wd_trace_t temp[64];
    int n = wave_index_select(dl->wave_index, wish->rhythm, wish->rhythm_mask, pmin, pmax, temp, max_results);
    for (int i = 0; i < n; i++) {
...
static int wish_ingenuity(dreamland_t *dl, const wish_t *wish, dream_result_t *results, int max_results) {
    ...
    for (int i = 0; i < n_spaces; i++) {
        results[0].source_ids[i] = spaces[i].source_rhythm;
    }
