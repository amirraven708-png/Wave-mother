    ...
    FILE *logfile)
{
    ...
    for (int iter = 0; iter < max_iters; iter++) {
        ...
        if (logfile) {
            log_trajectory(
                logfile, seed, iter,
                residual, residual - old_residual,
                coherence, coherence - old_coherence,
                N, best_stats.value, best_stats.weight,
                kp->capacity,
...
int main(void)
{
    ...
    FILE *trajectory_all = fopen("psi_trajectory_100seed.csv", "w");
    if (trajectory_all) {
        fprintf(trajectory_all, 
            "seed,iteration,residual,delta_residual,coherence,delta_coherence,N,value,weight,capacity,feasible\n");
    }
    
    printf("╔══════════════════════════════════════════════════════╗\n");
    printf("║     PSI-CORE 100-SEED BENCHMARK                     ║\n");
    printf("╚══════════════════════════════════════════════════════╝\n\n");
    ...
    for (int s = 0; s < NUM_SEEDS; s++) {
        ...
        benchmark_result_t res = knapsack_psi_solve_seed(&kp, seed, dp_val, trajectory_all);
        results[s] = res;
    ...
    if (trajectory_all) fclose(trajectory_all);
    
    // Sort gaps for median calculation
    ...
    printf("\n╔══════════════════════════════════════════════════════╗\n");
    printf("║     PSI-CORE 100-SEED RESULTS                       ║\n");
    printf("╚══════════════════════════════════════════════════════╝\n\n");
    ...
    FILE *summary = fopen("psi_100seed_summary.csv", "w");
    if (summary) {
        fprintf(summary, "seed,dp_value,psi_value,gap_percent,iterations,repairs,final_N,final_coherence,final_residual,time_ms,exact_hit,feasible\n");
        for (int i = 0; i < NUM_SEEDS; i++) {
            fprintf(summary, "%d,%d,%d,%.4f,%d,%d,%.2f,%.4f,%.4f,%.2f,%d,%d\n",
                results[i].seed,
                results[i].dp_value,
                results[i].psi_value,
        ...
        fclose(summary);
        printf("📁 Detailed results saved to psi_100seed_summary.csv\n");
    }
    
    printf("📁 Complete trajectory saved to psi_trajectory_100seed.csv\n");
    printf("   (%.0f data points across %d seeds)\n", 
           total_repairs + NUM_SEEDS * 10.0, NUM_SEEDS);
